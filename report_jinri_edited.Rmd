...
setwd("C:/Users/jhj73_000/Desktop/고대/진리장학금/모델")
```
...
dat <- read.csv("finaldataset_dong.csv", header=T)
hist(dat$landvalue, main = "동별 평균 지가 분포", xlab = "동별 지가")
```
...
```{r lmfit_step, include=FALSE}
forward <- step(lmFit_null, direction = "forward", scope = formula(lmFit))
backward <- step(lmFit, direction = "backward", scope = formula(lmFit_null))
stepwise <- step(lmFit, direction = "both")
```

 - 변수선택 결과
```{r lmfit_step_summary}
summary(forward)
summary(backward)
summary(stepwise)
...
```{r tree_plot, echo=FALSE}
par(mfrow=c(1, 1))
plot(tree.fit)
text(tree.fit, cex=.75)  # cex = 글씨 크기 배율
```
